...
class ModelName(Enum):
    ...
    CHATGPT_4O_LATEST = "gpt-4o-2024-11-20"
...
def main(datas: List[Tuple[str, List[Dict]]]):
    ...
    for items in zip(*data_lists):
        ...
        resps = [parse_rank_string_to_code(item['response']) for item in items]
...
if __name__ == "__main__":
    ...
    for benchmark in benchmarks:
        benchmark_path = os.path.join(ROOT_PATH, benchmark, 'ranks')
        dirs = os.listdir(benchmark_path)
        number_models = []
        for dir in dirs:
            model_name = ModelName.get_model_name(dir).value
            if model_name == 'error':
                continue
            number_model = bench_capability[benchmark].get(model_name)
            number_models.append((model_name, number_model))
        model_sorted = sorted(number_models, key=lambda x: x[1])
        model_dict_sorted = [(model_name, os.path.join(benchmark_path, model_name)) for model_name, _ in model_sorted]
        datas = []
        for model_name, model_path in model_dict_sorted:
            # Find the _rank.jsonl file in the model directory
            rank_files = [f for f in os.listdir(model_path) if f.endswith('.jsonl')]
            if not rank_files:
                print(f"Warning: No jsonl file found in {model_path}")
                continue
            raw_rank_path = os.path.join(model_path, rank_files[0])
            data = read_jsonl_file(raw_rank_path)
            datas.append((model_name, data))
        ranks = main(datas)
        output_path = os.path.join('outputs', benchmark, 'input', 'ranks.json')
