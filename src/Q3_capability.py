...
if __name__ == "__main__":
    ...
    method = 'kemeny_young'
    model_macro_pearson = {bench: np.nan for bench in benchmarks}
    model_micro_pearson = {bench: [] for bench in benchmarks}
    for bench in benchmarks:
        ...
        for rank_df in tqdm(rank_matrixs):   # 每道题目
            ...
            if rerank_result is None:
                model_micro_pearson[bench].append(np.nan)
                continue
            model_micro_pearson[bench].append(pearsonr(rerank_result, chatbot_arena_rank)[0])
            all_question_rank.append(rerank_result)
        all_question_rank = [_ for _ in all_question_rank if _]
        all_quest_avg = np.nanmean(all_question_rank, axis=0)
        model_macro_pearson[bench] = pearsonr(all_quest_avg, chatbot_arena_rank)[0]

    # model_micro_pearson 填充不足的值
    max_len = max(len(p) for p in model_micro_pearson.values())
    for bench in benchmarks:
        model_micro_pearson[bench].extend([np.nan] * (max_len - len(model_micro_pearson[bench])))
    model_micro_pearson_df = pd.DataFrame(model_micro_pearson)
    model_macro_pearson_df = pd.DataFrame(model_macro_pearson, index=['macro_pearson_correlation'])
    pearson_output_dir = f'Q3_capability/{method}'
    os.makedirs(pearson_output_dir, exist_ok=True)
    model_micro_pearson_df.to_excel(f'{pearson_output_dir}/micro_pearson_corr.xlsx')
    model_micro_pearson_df.describe().to_excel(f'{pearson_output_dir}/micro_pearson_corr_describe.xlsx')
    model_macro_pearson_df.to_excel(f'{pearson_output_dir}/macro_pearson_corr.xlsx')
